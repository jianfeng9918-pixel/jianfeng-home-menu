...
(() => {
  ...
  $('open-sources').addEventListener('click',()=>{
    ...
    $('source-list').innerHTML='<p>图片为菜品示意，实际做法和摆盘以家里制作的为准。</p>'+sources.map(([name,img])=>`<article><h3>${esc(name)}</h3><p>${esc(img.author||'来源见原文')}${img.license&&img.license!=='unknown'?` · ${esc(img.license)}`:''}</p>${img.sourcePage?`<a href="${esc(img.sourcePage)}" target="_blank" rel="noopener noreferrer">查看原始来源</a>`:''}${img.licenseUrl?` · <a href="${esc(img.licenseUrl)}" target="_blank" rel="noopener noreferrer">使用许可</a>`:''}</article>`).join('')+'<article><h3>V10 三张食材构图精修</h3><p>家里招牌、鸡肉与蛋、鱼虾海鲜食材图由内置 ImageGen 生成；家里招牌本次复用此前已生成的木台构图，其他栏目图保留。麦穗短句和招牌标签由网页绘制。</p><a href="V10图片生成记录.json" target="_blank" rel="noopener noreferrer">查看 V10 提示词与生成记录</a></article><article><h3>V9 栏目食材图</h3><p>十张食材画面由内置 ImageGen 生成，栏目文字、暖金招牌标签和绿色卖点条由网页排版。门头继续使用家宴照片。</p><a href="V9图片生成记录.json" target="_blank" rel="noopener noreferrer">查看提示词与生成记录</a></article><article><h3>此前菜品图记录</h3><a href="V8图片生成记录.json" target="_blank" rel="noopener noreferrer">查看 V8 生成记录</a></article>';openDialog('sources-dialog');
  });
